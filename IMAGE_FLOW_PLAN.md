...
> 状态：方案评审稿 + Phase 0 实测已完成，**核心实现已落地**（Phase 1-4）。
> 日期：2026-08-02
...
> **2026-08-02 更新：** 下述方案（注册表 / 预取 / 详情拆分 / 截图兜底 / 并发池）已按 Phase 1-4 落地，见 [§10 实施阶段](#10-实施阶段)。实现细节与方案差异见各节备注。
...
2. **Phase 1 ✅ 已完成**：注册表 `JavCinema.imageUrlsRegistry` + `fromApiList` 填充（含欧美 `posterLarge` 直取，顺带修复 ps→pl 失效）+ `MovieCard` 占位 + 可见性预取。
   - 实测：`AVMOProvider.fromApiList()`（`AVMOProvider.kt:144`）将 `movieId → ImageUrls(posterSmall, posterLarge, sampleSmall, sampleLarge)` 写入 `JavCinema.imageUrlsRegistry`。
   - `MovieCard.kt:57-74` 可见时预取大封面：注册表直取 `posterLarge`，未命中回退 `ps→pl` 推断。
3. **Phase 2 ✅ 已完成**：详情并发改造（大封面直取 `posterLarge` 写主内存缓存；截图缩略图注册表驱动；女优串行；关联列表并发；回写注册表）。
   - 实测：`detail` 截图加载与封面加载拆分（`screenshotImageLoader` 独立池 maxRequestsPerHost=2）；`loadDetail` 去重防 NavHost 重组双请求（commit 58152b6）。
4. **Phase 3 ✅ 已完成**：占位比例对齐实测（列表 `aspectRatio(0.7)`、详情大封面横版占位）、blur 动画（`animateFloatAsState(20→0, tween(600))`）、导航过渡。
5. **Phase 4 ✅ 已完成**：退出回收 + 注册表清理（预取随 VM/DisposableEffect 取消）。
6. **Phase 5 ⏳ 待做**：`./gradlew.bat assembleDebug` + `./gradlew test` + 弱网/快速进出/切数据源回归；真机验证"立即回显"与"成功一个回显一个"。
